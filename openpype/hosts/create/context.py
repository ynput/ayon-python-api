...
class CreatedInstance:
    ...
    about instance like "folder" and "task" and all data used for filling subset
    ...
    def set_folder_invalid(self, invalid):
        # TODO replace with `set_folder_name`
...
class CreateContext:
    ...
    def reset_plugins(self, discover_publish_plugins=True):
        ...
        plugins.extend(
            discover_class(self._registered_plugin_paths, BaseCreator)
        )

        creators = {}
        autocreators = {}
        manual_creators = {}
        for creator_class in plugins:
    ...
    def validate_instances_context(self, instances=None):
        ...
        task_names_by_folder_id = {}
        for instance in instances:
            task_name = instance.get("task")
            folder_id = instance.get("folder")
            if folder_id:
                task_names_by_folder_id[folder_id] = set()
                if task_name:
                    task_names_by_folder_id[folder_id].add(task_name)

        folder_ids = [
            folder_id
            for folder_id in task_names_by_folder_id.keys()
            if folder_id is not None
        ]
        # TODO implement rest of the logic of validation
